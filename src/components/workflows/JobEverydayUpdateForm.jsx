...
export default function JobEverydayUpdateForm({
    job = null,
    allJobs = [],
    onClose,
    onSave,
    ...
    useEffect(() => {
        if (selectedJob) {
            ...
            // Prefill next action if existing
        ...
        try {
            ...
            if (dailyNote.trim() || nextActionDate) {
                ...
                if (syncWithGoogleCalendar && nextActionDate) {
                    ...
                    await triggerCalendarEventCreation({
                        title: calTitle,
                        description: calDesc,
            ...
            const jobDocPayload = {
                ...
                delivery_verification: {
                    ...
                    last_daily_note: dailyNote.trim() || existingDv.last_daily_note || ''
                }
            };
    ...
    const content = (
        <div style={{
            ...
            {/* Header with Mode Toggle & Status Badge */}
            <div style={{
                padding: '16px 20px',
                    ...
                    <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                            <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 800, color: '#f8fafc' }}>
                                {mode === 'new' ? 'New Job Entry Form' : `Job Everyday Update — ${formData.document_no}`}
                            </h3>
                            <span style={{
                                fontSize: '0.72rem',
                                fontWeight: 700,
                        ...
                        <p style={{ margin: 0, fontSize: '0.76rem', color: '#94a3b8' }}>
                            Fill details, update live status tells, log everyday progress notes, and sync to Google Calendar
                        </p>
                    </div>
                </div>
                    ...
                    {allJobs && allJobs.length > 0 && (
                        <div style={{ display: 'flex', background: 'rgba(255,255,255,0.1)', borderRadius: '8px', padding: '2px' }}>
                            <button
                                ...
                                style={{
                                    background: mode === 'new' ? '#3b82f6' : 'transparent',
                                    color: '#ffffff',
                                    border: 'none',
                                    padding: '5px 12px',
                                    fontSize: '0.75rem',
                            ...
                            <button
                                ...
                                style={{
                                    background: mode === 'existing' ? '#3b82f6' : 'transparent',
                                    color: '#ffffff',
                                    border: 'none',
                                    padding: '5px 12px',
                                    fontSize: '0.75rem',
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                            ...
                            <span style={{ fontSize: '0.72rem', color: '#64748b' }}>
                                Click any status pill to update the job's current tell
                            </span>
                        </div>
                        ...
                        {mode === 'existing' && (
                            <>
                                ...
                                {onNavigateToJob && formData.id && (
                                    <button
                                        type="button"
                                        onClick={() => onNavigateToJob(formData.id)}
                                        className="btn btn-secondary"
